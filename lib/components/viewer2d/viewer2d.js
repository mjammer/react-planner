...
function Viewer2D(_ref, _ref2) {
  ...
  var onMouseUp = function onMouseUp(viewerEvent) {
    ...
    switch (mode) {
      ...
      case constants.MODE_DRAWING_CONNECTION:
        {
          var _elementData = extractElementData(event.target);
          if (_elementData && _elementData.prototype === 'items') {
            connectionsActions.endDrawingConnection(_elementData.layer, _elementData.id);
          } else {
  ...
  return _react2.default.createElement(
    ...
    _react2.default.createElement(
      ...
      _react2.default.createElement(
        ...
        mode === constants.MODE_DRAWING_CONNECTION && function () {
          var drawingSupport = state.drawingSupport;
          var startItemId = drawingSupport.get('startItemId');
          var drawingLayerID = drawingSupport.get('layerID') || layerID;
          var currentX = drawingSupport.get('currentX');
          var currentY = drawingSupport.get('currentY');
          var startItem = startItemId && state.getIn(['scene', 'layers', drawingLayerID, 'items', startItemId]);
...
Viewer2D.contextTypes = {
  ...
  connectionsActions: _propTypes2.default.object.isRequired,
